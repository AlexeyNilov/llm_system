...
def _world() -> ValidatedWorldState:
    ...
    return validate_world_state(
        ...
        WorldState(
            ...
            characters=(
                CharacterState(character_id="marin", location_id="remote"),
...
def test_matching_past_and_current_speech_retains_exact_events_after_movement() -> None:
    past = _speech(1, occurred_at_seconds=12)
    current = _speech(2)

    feedback = project_addressed_speech_feedback(_world(), "sela", (past, current))
    ...
    assert feedback[0].event is past
    assert feedback[1].event is current


def test_only_exact_recipient_speech_matches_and_self_ownership_stays_speaker_only() -> (
    None
):
    matching = _speech(1)
    other_recipient = _speech(2, recipient_id="tovan")
    observer_speaking = _speech(3, speaker_id="sela", recipient_id="tovan")
    ...
    feedback = project_addressed_speech_feedback(_world(), "sela", events)

    assert tuple(item.event for item in feedback) == (matching,)
    assert project_self_event_feedback(_world(), "sela", (matching,)) == ()
    assert tuple(
        item.event
        for item in project_self_event_feedback(_world(), "sela", (observer_speaking,))
    ) == (observer_speaking,)


def test_observer_validation_precedes_whole_batch_future_validation() -> None:
    future_other_recipient = _speech(1, recipient_id="tovan", occurred_at_seconds=31)

    for observer_id in ("unknown", "token"):
        with pytest.raises(PerceptionObserverNotFoundError) as observer_error:
            project_addressed_speech_feedback(
                _world(), observer_id, (future_other_recipient,)
            )
        assert observer_error.value.observer_id == observer_id

    with pytest.raises(FutureEventFeedbackError) as future_error:
        project_addressed_speech_feedback(
            _world(), "sela", (_speech(2), _waited(31), _speech(3))
        )
    assert future_error.value.event_id == _waited(31).event_id
...
def test_projection_preserves_order_duplicates_empty_results_and_inputs() -> None:
    world = _world()
    first = _speech(1, occurred_at_seconds=10)
    second = _speech(2)
    ...
    projected = project_addressed_speech_feedback(world, "sela", events)
    ...
    assert projected == project_addressed_speech_feedback(world, "sela", events)
    assert project_addressed_speech_feedback(world, "sela", ()) == ()
    assert project_addressed_speech_feedback(world, "sela", (_waited(),)) == ()
