...
* Actor-action operation dispatch: a pure post-authorization, pre-resolution routing boundary that selects implemented resolvers by concrete proposal type and forwards caller-injected outcome and event identities. TASK-034 extends the initial partial dispatcher from Move, Wait, Observe, Speak, and Take to the accepted bound Use resolver; Help retains a typed unavailable-capability error rather than producing a canonical outcome.
